...
class DatasetCOCO(Dataset):
    ...
    def __init__(self, datapath, fold, transform, split, way, shot):
        ...
        self.way = way
        self.shot = shot
        self.split_coco = split if split == "val2014" else "train2014"  # never called
    ...
    def __getitem__(self, idx):
        ...
        query_class_presence = [
            s_c in torch.unique(query_cmask) for s_c in _support_classes
        ]  # needed - 1
        rename_class = (
            lambda x: _support_classes.index(x) + 1 if x in _support_classes else 0
        )
        ...
        support_imgs = torch.stack(
            [
                torch.stack(
                    [self.transform(support_img) for support_img in support_imgs_c]
                )
                for support_imgs_c in support_imgs
            ]
        )
        support_masks = self.get_support_masks(
            support_imgs, _support_classes, support_cmasks, rename_class
        )
    ...
    def get_query_mask(self, query_img, query_cmask, rename_class):
        if (
            self.split == "trn"
        ):  # resize during training and retain orignal sizes during validation
            query_cmask = F.interpolate(
                query_cmask.unsqueeze(0).unsqueeze(0).float(),
                query_img.size()[-2:],
                mode="nearest",
            ).squeeze()
        query_mask = self.generate_query_episodic_mask(
            query_cmask.float(), rename_class
        )
        return query_mask

    def get_support_masks(
        self, support_imgs, _support_classes, support_cmasks, rename_class
    ):
        support_masks = []
        for class_id, scmask_c in zip(_support_classes, support_cmasks):  # ways
            ...
            for scmask in scmask_c:  # shots
                ...
                support_mask = self.generate_support_episodic_mask(
                    scmask, class_id, rename_class
                )
                assert (
                    len(torch.unique(support_mask)) <= 2
                ), f"{len(torch.unique(support_mask))} labels in support"
    ...
    def load_frame(self, query_name, support_names):
        ...
        support_imgs = [
            [self.read_img(name) for name in support_names_c]
            for support_names_c in support_names
        ]
        support_masks = [
            [self.read_mask(name) for name in support_names_c]
            for support_names_c in support_names
        ]
    ...
    def sample_episode(self, idx):
        ...
        query_name = np.random.choice(
            self.img_metadata_classwise[query_class], 1, replace=False
        )[0]
        ...
        p = np.ones([len(self.class_ids)]) / 2.0 / float(len(self.class_ids) - 1)
        p[self.class_ids.index(query_class)] = 1 / 2.0
        support_classes = np.random.choice(
            self.class_ids, self.way, p=p, replace=False
        ).tolist()
        print(f">>>> support_classes: {support_classes}")
        for sc in support_classes:
            support_names_c = []
            while True:  # keep sampling support set if query == support
                support_name = np.random.choice(
                    self.img_metadata_classwise[sc], 1, replace=False
                )[0]
